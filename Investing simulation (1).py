...
## next I plotted the first 10 simulations to visualize the data
...
## Summarize the final results from the last row of the table 
...
## created a histogram of the final results to visualize the distrubution of ending balances

plt.hist(lastRow, bins=100)
...
## Probability we get less than 1 million

len(lastRow[lastRow>1000000]) / len(lastRow)


# In[11]:


## table of values from numpys percentile function

p_tiles = np.percentile(lastRow, [10,20,30,40,50,60,70,80,90])
for p in range(len(p_tiles)):
    l = [10,20,30,40,50,60,70,80,90]
    print ( '{} percentile:'.format(l[p]).rjust(15), '{}'.format(locale.currency(p_tiles[p], grouping=True)))
